...
void setup() {
  ...
  pinMode(selectPin, INPUT_PULLUP); //internal pullup input on sample select button, NEED TO CHANGE FOR CDR
  attachInterrupt(digitalPinToInterrupt(selectPin), sampleSelect, LOW);//trigger sample select function ISR on button press not release NEED TO CHANGE FOR CDR
...
//    }
  String sample = curSample.name(); //NEED TO CHANGE FOR CDR, cannot use String for CDR
...
//    }
  String nextEntry = nextSample.name(); //NEED TO CHANGE FOR CDR cannot use String
